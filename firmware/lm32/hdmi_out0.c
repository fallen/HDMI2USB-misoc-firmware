...
void hdmi_out0_print_edid(void) {
    ...
    for(e = 0; e < extension_number; e++)
    {
        ...
        sum = 0;
        for (eeprom_addr = 0 ; eeprom_addr < EDID_BLOCK_SIZE ; eeprom_addr++) {
            b = hdmi_out0_i2c_read(eeprom_addr == EDID_BLOCK_SIZE-1 && e == extension_number - 1 ? 0 : 1);
            sum += b;
            printf("%02X ", b);
            if(!((eeprom_addr+1) % 16))
                printf("\n");
